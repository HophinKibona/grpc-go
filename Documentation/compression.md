...
servers is to use
[`encoding.RegisterCompressor`](https://godoc.org/google.golang.org/grpc-forked/encoding#RegisterCompressor)
...
it via the
[`UseCompressor`](https://godoc.org/google.golang.org/grpc-forked#UseCompressor)
`CallOption`.  Remember that `CallOption`s may be turned into defaults for all
calls from a `ClientConn` by using the
[`WithDefaultCallOptions`](https://godoc.org/google.golang.org/grpc-forked#WithDefaultCallOptions)
