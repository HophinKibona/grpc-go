...
go build -o ${out_dir}/server $GOPATH/src/google.golang.org/grpcv54/benchmark/server/main.go && go build -o ${out_dir}/client $GOPATH/src/google.golang.org/grpcv54/benchmark/client/main.go
if [ $? != 0 ]; then
  clean_and_die 1
fi
